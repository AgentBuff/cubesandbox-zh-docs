...
标准 one-click 构建需要准备**普通 guest 内核** `vmlinux`（自行编译或使用预编译版本），放置到指定目录：
...
如果你要构建的是 PVM 发布包，而不是普通 KVM 路径，请额外准备 **PVM guest 内核** `vmlinux-pvm`。仓库已提供专用脚本：
...
- `build-pvm-guest-vmlinux.sh` 产出 one-click 可直接打包的 PVM guest 内核
- `build-pvm-host-kernel-pkg.sh` 产出 PVM 宿主机内核安装包
- 若希望一条命令串起构建、安装、GRUB 配置和 guest kernel 投放，可改用 `./deploy/pvm/pvm_setup.sh`

PVM 交付细节请参阅 [PVM 部署](./pvm-deploy.md)。
...
| `ONE_CLICK_CUBE_KERNEL_PVM_VMLINUX` | 空 | 可选的 PVM guest 内核（`vmlinux-pvm`）路径；构建 PVM 发布包时使用 |
...
| `CUBE_PVM_ENABLE` | `0` | 是否在安装时把打包好的 PVM guest 内核安装为运行时 `cube-kernel-scf/vmlinux` |
...
- 如果要交付 PVM 发布包但未提供 `ONE_CLICK_CUBE_KERNEL_PVM_VMLINUX`，发布包中不会带上 `vmlinux-pvm`，目标机也无法通过 `CUBE_PVM_ENABLE=1` 切换到 PVM guest 内核。

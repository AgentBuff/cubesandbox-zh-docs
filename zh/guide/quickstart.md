...
::: tip 云服务器没有 `/dev/kvm`？
如果你的机器是普通云服务器、宿主 hypervisor 没有开放嵌套虚拟化，请不要继续本页流程，直接改看 [PVM 部署](./pvm-deploy.md)。PVM 路径使用“PVM 宿主机内核 + PVM guest 内核（`vmlinux-pvm`）+ `CUBE_PVM_ENABLE=1`”这套组合能力。
:::
...
- [PVM 部署](./pvm-deploy.md) — 当目标机没有 `/dev/kvm` 时的替代部署路径

...
- PVM 相关：`deploy/pvm/*.sh`、PVM 宿主机内核、PVM guest 内核（`vmlinux-pvm`）与 one-click 启用变量
- 命令行入口：`cubemastercli`、`cubecli`、`cube-runtime`
- 关键配置：仓库模板、安装后配置、运行时生成文件、健康检查与日志路径

## 阅读顺序

1. [命令总览与 CLI 参考](./01-command-reference.md)：所有主要命令与 CLI 入口
2. [控制面配置文件路径与说明](./02-control-plane-config.md)：控制面组件配置文件路径与说明
3. [节点与运行时配置文件路径与说明](./03-node-runtime-config.md)：节点侧与运行时配置文件路径与说明
4. [安装目录、生成文件与落盘路径](./04-install-layout-and-generated-files.md)：安装目录、生成文件、落盘路径
5. [日志、健康检查与排障](./05-logs-health-and-troubleshooting.md)：日志、巡检、排障

...
1. 安装 PVM 宿主机内核并重启
2. 安装 Cube Sandbox 时启用 `CUBE_PVM_ENABLE=1`，让运行时切换到 PVM guest 内核（`vmlinux-pvm`）
...
如果你不使用 Release 附件，而是希望自己构建 PVM 产物，仓库里已经提供了完整脚本：
...
- **PVM 宿主机内核**：安装到目标机并作为默认启动内核，提供 `kvm_pvm`
- **PVM guest 内核（`vmlinux-pvm`）**：打进 one-click 发布包，最终在启用 PVM 时安装为运行时 `cube-kernel-scf/vmlinux`

如果希望把构建、安装、GRUB 配置和 guest 内核投放串成一条链路，可直接执行：
...
构建 one-click 发布包时，如果要显式注入 PVM guest 内核，可设置：
...
发布包中包含两份 guest 内核：普通 guest 内核（`vmlinux`）和 PVM guest 内核（`vmlinux-pvm`）。`CUBE_PVM_ENABLE=1` 告知安装脚本将 PVM guest 内核覆盖安装为运行时使用的 `vmlinux`。不设置此变量则默认使用普通 guest 内核，PVM 不会生效。
:::
...
# 确认运行时使用的仍是统一路径，只是内容已切换为 PVM guest 内核
...
如果你是自建发布包，还要额外确认构建时是否提供了 `ONE_CLICK_CUBE_KERNEL_PVM_VMLINUX`，否则包里根本不会带上 `vmlinux-pvm`。
...
**Q3：我已经切到 PVM 宿主机内核了，还需要重新做模板吗？**

**A：** 通常不需要。模板、镜像、API 使用方式与标准部署一致。PVM 变化的是底层虚拟化路径，以及运行时使用的 guest 内核。
